...
function runTests() {
    ...
    console.assert(exclusiveResult.baseAmount === 1000, 'Invoice amount mismatch');
    console.assert(exclusiveResult.gstAmount === 180, 'GST amount mismatch');
    console.assert(exclusiveResult.totalAmount === 1180, 'Outstanding mismatch');
    ...
    console.assert(Math.abs(inclusiveResult.baseAmount - 1000) < 0.01, 'Invoice amount mismatch');
    console.assert(Math.abs(inclusiveResult.gstAmount - 180) < 0.01, 'GST amount mismatch');
    console.assert(inclusiveResult.totalAmount === 1180, 'Outstanding mismatch');
    ...
    console.assert(noGstResult.baseAmount === 1000, 'Invoice amount mismatch');
    console.assert(noGstResult.gstAmount === 0, 'GST amount mismatch');
    console.assert(noGstResult.totalAmount === 1000, 'Outstanding mismatch');
